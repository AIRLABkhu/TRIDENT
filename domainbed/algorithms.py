...
class get_baseline(Algorithm):
    def __init__(self, input_shape, num_classes, num_domains, hparams):
        super(get_baseline, self).__init__(input_shape, num_classes, num_domains,
                                  hparams)
...
class ERM(Algorithm):
    ...
    def __init__(self, input_shape, num_classes, num_domains, hparams):
        ...
        # params = [
        #     {'params': self.featurizer.parameters(), 'lr': 0.003},
...
class DDG(Algorithm):
    def __init__(self, input_shape, num_classes, num_domains, hparams):
        super(DDG, self).__init__(input_shape, num_classes, num_domains,
                                  hparams)
        self.network = networks.timm_network(input_shape, self.hparams, num_classes, self.hparams['split_layer'])
        feat_dim = self.network.inter_feat_dim
        inter_shape = self.network.inter_shape[self.hparams['split_layer']]
        # self.diff = networks.LatentDiffusion(inter_shape, feat_dim, ae_enable=self.hparams['ae_enable'])
        self.diff = networks.SimpleLatentDiffusion(inter_shape, feat_dim, ae_enable=self.hparams['ae_enable'])

        self.parameters = (list(self.network.featurizer.parameters()) \
            + list(self.network.classifier.parameters()) \
            + list(self.diff.parameters()))
        # self._check_grad_param()
    ...
    def update(self, minibatches, unlabeled=None):
        all_x = torch.cat([x[0] for x, y in minibatches])
        all_aug = torch.cat([x[1] for x, y in minibatches]) # it would be aug
        all_y = torch.cat([y for x, y in minibatches])

        all_feats = self.network.featurizer(all_x)
        all_aug_feats = self.network.featurizer(all_aug)

        # ddpm
        ddpm_loss, rec_loss = self.diff.forward_train(all_feats.detach(), all_aug_feats.detach())

        pred_y = self.network.classifier(all_feats)
        ce_loss = F.cross_entropy(pred_y, all_y)
        loss = ddpm_loss + rec_loss + ce_loss

        self.optimizer.zero_grad()
        loss.backward()
        self.optimizer.step()

        if self.hparams['ae_enable']:
            return {'total_loss': loss.item(), 'loss_ddpm': ddpm_loss.item(), 'loss_rec': rec_loss.item(), 'loss_ce': ce_loss.item()}
        else:
            return {'total_loss': loss.item(), 'loss_ddpm': ddpm_loss.item(), 'loss_ce': ce_loss.item()}

    def predict(self, x):
        x = self.network.featurizer(x)
        x = self.diff.forward_pred(x)
