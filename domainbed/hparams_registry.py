...
def _hparams(algorithm, dataset, random_seed):
    ...
    _hparam('data_augmentation_root', '', lambda r: '')
    _hparam('undersample_ratio', 1.0, lambda r: r.choice([0.1, 0.25, 0.5, 0.75, 1.0]))
    _hparam('data_randomsample_root', None, lambda r: None)
    _hparam('resnet18', False, lambda r: False)
    _hparam('resnet50_augmix', False, lambda r: False)
    _hparam('dinov2', False, lambda r: False)
    _hparam('small', False, lambda r: False)
    ...
    _hparam('load_pretrained', None, lambda r: None)
    ...
    # for DDG
         ...
         _hparam('multi_augment', True, lambda r: True)
         _hparam('ae_enable', True, lambda r: True)
         _hparam('split_layer', 4, lambda r: r.choice([None, 1, 2, 3, 4]))
         _hparam('learnable_param', 'whole', lambda r: r.choice(['whole', 'classifier', 'header']))
    ...
    if algorithm == "ADRMX":
        ...
        hparams["rho"] = (0.05, lambda r: r.choice([0.01, 0.02, 0.05, 0.1]))
        hparams["alpha"] = (0.001, lambda r: r.choice([0.01, 0.02, 0.05, 0.1]))
    ...
    if dataset in SMALL_IMAGES:
        ...
        if algorithm == "ADRMX":
            ...
            _hparam('lr', 1e-4, lambda r: 10**r.uniform(-4.5, -4))
        else:
    ...
    if dataset in SMALL_IMAGES:
        ...
        if algorithm == "DDG":
            _hparam('weight_decay', 0.01, lambda r: 0.01)
        else:
