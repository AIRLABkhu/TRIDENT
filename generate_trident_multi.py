...
from src.models.trident import TRIDENT
...
class DomainGenerator:
    def __init__(
        ...
        if save_dir is not None:
            self.data_save_dir = os.path.join(save_dir)
        ...
        self.pipe = StableUnCLIPImg2ImgPipeline.from_pretrained(
                STABLE_DIFFUSION_MODEL, torch_dtype=torch.float16, force_download=True
            ).to(self.device)
    ...
    def generate(
        ...
        for batch in range(len(os.listdir(self.src_dir))):
            src_filename1, src_data1, src_img_raw1, src_filename2, src_data2, src_img_raw2 = next(data_store)
            label = None
            if isinstance(src_data1, dict):
                src_img1 = src_data1["img"]
                src_img2 = src_data2["img"]
            else:
                src_img1 = src_data1
                src_img2 = src_data2
 
            src_emb1 = self.visual_encoder(src_img1).image_embeds
            src_emb1 = src_emb1.type(torch.float32)
            dom_emb1, cls_emb1, att1, dom_att1, cls_att1 = split_model(src_emb1)

            src_emb2 = self.visual_encoder(src_img2).image_embeds
            src_emb2 = src_emb2.type(torch.float32) 
            dom_emb2, cls_emb2, att2, dom_att2, cls_att2 = split_model(src_emb2) 

            ref_dom_att1, ref_cls_att1 = split_model.refined_attribute(dom_emb2, cls_emb1, att1)
            # ref_dom_att2, ref_cls_att2 = simple_layer.refined_attribute(dom_emb1, cls_emb2, att2)
            # dom2_att_vector = split_model.refined_attribute(src_emb, domain_vector2)

            src_emb = dom_emb2 + cls_emb1 + ref_dom_att1 + ref_cls_att1
            ...
            for i, img in enumerate(imgs):
                img.save(
                    os.path.join(self.data_save_dir, f"{batch}_{i}_{src_filename1}")
                )
...
def main():
    ...
    print(f"simple_model loaded {weight_path}")
